...
export default function MacWindow({
  ...
  const { close, minimize, appWindowRef } = useWindows();
  ...
  const resize = (dir: string) => (e: React.MouseEvent) => {
    ...
    const move = (ev: MouseEvent) => {
      ...
      if (dir.includes("right")) newBox.w = Math.max(200, startBox.w + dx);
      ...
      if (dir.includes("bottom")) newBox.h = Math.max(150, startBox.h + dy);
  ...
  return (
    ...
    >
      ...
      <div
        ref={appWindowRef}
        className="h-[calc(100%-40px)] rounded-2xl overflow-auto custom-scrollbar relative @container"
      >
        {children}
      </div>

      {/* Resize handles */}
      {["top", "right", "bottom-right", "bottom", "bottom-left", "left"].map(
        (dir) => (
          <div
            key={dir}
